...
function detailSetVideoHight() {
    setTimeout(function(){
        var contentW = $('.rich_media_content').width();
        $('video').each(function(index, item){
            vW = $(item)[0].videoWidth;
            vH = $(item)[0].videoHeight
            if (vW > vH) {
                $(item).css('height', contentW*vH/vW);
            } else {
                $(item).css('height', '50.63vw')
            }
        })
    }, 100);
}